...
files = ["output.xyz", "del_output.xyz"] #list of files for upload
host_portal = "http://localhost/api"

with requests.Session() as s:
    s.headers['Authorization'] = f"Token {my_pointerra_api_key}"
    response = s.post(host_portal + "/jobs/", json={"name":"output","source_job":None,"dataset_type":"not_specified","feature_coding_scheme_id":None,"description":"","attribution":"","extra":"","extra_args":None,"vertical_datum":"geoid","horizontal_to_meters":1,"vertical_to_meters":1,"map_coordinate":None,"process_scans_into_layers":False,"skip_deleted_points":True,"only_visible_points":True,"include_user_edits":True,"sub_model_size":7.5,"replaces_pointcloud":None,"remove_long_term_archive":True,"email_notification":True,"hidden":False,"source_data_uri":"","company_id":"","custom_srs":None,"custom_srs_name":None,"source_srs":None,"no_srs":True,"date_acquired":None,"layers":{"default":[]},"raw_data_size":0,"processing_resolution":0.0015})
    print (response.json())
    response_data = response.json()
    upload_response = s.post(host_portal + "/jobs/upload_request", json={"job_id": response_data["job_id"] ,"file_manifest":{"files":files,"datasets":[]}})
...
with requests.Session() as s: 
    ...
    s.headers['Authorization'] = f"Token {my_pointerra_api_key}"
    job_posted_response = s.patch(host_portal + "/jobs/" + response_data["job_id"], json={"status":"pending","deleted_files":[],"extra_args":None})
